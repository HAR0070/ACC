...
def kalman(A,B,x,P,u,z_odometry):
    ...
    R_odometry = np.array([[0.08, 0],
                           [0, 0.15]])  # Odometry measurement noise covariance

    Q = np.array([[0.2, 0],
                  [0, 0.2]]) # Define process noise covariance
...
def solve():
    ...
    xk , var.P = kalman(Ak, Bk, var.x0[1:],var.P,var.a_cmd[var.iter],odometry)
    ...
    path , y , _ , _=simCon3(x0,var.y1,A,B,C,Np,Omega,Psi,Lz,M0, M1)
...
def publish_cmd():
    ...
    # var.v_cmd = var.ref[1] - var.y1[1]
    if var.iter <=8 and var.failure_iter ==0:
        i = var.i
